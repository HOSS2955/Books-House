          ...
          <div className="auth">
            ...
            <div className="mb-5">
              ...
              <InputGroup className="userInput mb-2" onBlur={onDisplayErrorMsg}>
                ...
                {/* Password input */}
                <Form.Control type="password" onChange={(e) => validate(e.target.value)} aria-label="Text input with checkbox" placeholder="Password" style={{ borderLeft: 'none', boxShadow: 'none' }} />
              </InputGroup>
              {/* for displaying the status of the input */}
              {displayErrorMsg || (<Form.Text className='errorMsg text-small float-start fw-semibold' style={{ color: errorMessage === "Accepted password enjoy reading :)" ? "green" : "#d21313" }}>
                {errorMessage}
              </Form.Text>)}
            </div>
            ...
            <button className="btn btn-dark mt-4 mb-2 fw-semibold logIn text-small" disabled={isDisabled}>Log in</button>
            <br></br>

            <a className="notYou text-dark fw-semibold text-small" href="#">Not you?</a>
